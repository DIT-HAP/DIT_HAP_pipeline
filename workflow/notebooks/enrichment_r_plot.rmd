...
enrichment_file <- "/data/c/yangyusheng_optimized/DIT_HAP_pipeline/results/HD_DIT_HAP/20_gene_level_clustering/go_enrichment_summary_for_manual_cluster_labels.tsv"
...
enrichment_data <- read_tsv(enrichment_file)
...
create_ridge_data <- function(feature_name, namespace) {
  ...
  ridge_data <- enriched_go_terms %>%
    select(GO, name, cluster, all_of(feature_name)) %>%
...
create_scatter_plot <- function(namespace, show_legend = FALSE) {
  plot_data <- enrichment_plot_data %>%
    filter(NS == namespace) %>%
    arrange(cluster_num, desc(neg_log_p))
  ...
  p <- ggplot(plot_data, aes(x = cluster_num, y = reorder(unique_name, cluster_num))) +
    geom_point(aes(size = coverage_frac, color = if("p_fdr_bh" %in% colnames(plot_data)) p_fdr_bh else adj_p), alpha = 0.8) +
...
create_ridge_plot <- function(feature_name, namespace, y_levels = NULL, show_legend = TRUE) {
  ...
  # Use the same factor levels as the scatter plot for consistent y-axis ordering
  if (!is.null(y_levels)) {
    ridge_data <- ridge_data %>%
      mutate(unique_name = factor(unique_name, levels = y_levels)) %>%
      # Remove rows where unique_name is not in the y_levels (to avoid orphaned data)
      filter(!is.na(unique_name))
  } else {
    ridge_data <- ridge_data %>%
      arrange(cluster_num) %>%
      mutate(unique_name = factor(unique_name, levels = unique(unique_name)))
  }
...
create_namespace_plot <- function(namespace) {
  ...
  # Get y-axis levels from scatter plot for consistent ordering
  y_levels <- levels(reorder(plot_data$unique_name, plot_data$cluster_num))
  
  # Create individual plots WITHOUT legends
  p1 <- create_scatter_plot(namespace, show_legend = FALSE)
  p2 <- create_ridge_plot("A", namespace, y_levels, show_legend = FALSE)
  p3 <- create_ridge_plot("um", namespace, y_levels, show_legend = FALSE)
  p4 <- create_ridge_plot("lam", namespace, y_levels, show_legend = FALSE)
  
  # Create plots WITH legends to extract legends
  p1_with_legend <- create_scatter_plot(namespace, show_legend = TRUE)
  p4_with_legend <- create_ridge_plot("lam", namespace, y_levels, show_legend = TRUE)
  
  # Extract legends
  scatter_legend <- get_legend(p1_with_legend)
  ridge_legend <- get_legend(p4_with_legend)
...
if ("BP" %in% unique(enriched_terms$NS)) {
  ...
  ggsave("/data/c/yangyusheng_optimized/DIT_HAP_pipeline/results/HD_DIT_HAP/21_similarity_analysis_for_terms/bp_enrichment_analysis.pdf", bp_plot, width = 16, height = 10)
} else {
...
if ("CC" %in% unique(enriched_terms$NS)) {
  ...
  ggsave("/data/c/yangyusheng_optimized/DIT_HAP_pipeline/results/HD_DIT_HAP/21_similarity_analysis_for_terms/cc_enrichment_analysis.pdf", cc_plot, width = 16, height = 10)
} else {
...
if ("MF" %in% unique(enriched_terms$NS)) {
  ...
  ggsave("/data/c/yangyusheng_optimized/DIT_HAP_pipeline/results/HD_DIT_HAP/21_similarity_analysis_for_terms/mf_enrichment_analysis.pdf", mf_plot, width = 16, height = 10)
} else {
...
summary_stats <- enriched_terms %>%
  ...
  summarise(
    ...
    median_coverage = median(coverage_frac),
    .groups = "drop"
  ) %>%
