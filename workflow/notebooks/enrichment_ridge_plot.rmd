...
# enrichment_file <- "/data/c/yangyusheng_optimized/DIT_HAP_pipeline/results/HD_DIT_HAP/20_gene_level_clustering/enrichment_analysis/selected_go_enrichment_summary_kmeans64mergedto8clusters.xlsx"
...
enrichment_data <- read_excel(enrichment_file)
...
create_ridge_data <- function(feature_name, namespace) {
  ...
  ridge_data <- enriched_go_terms %>%
    select(GO, name, cluster, cluster_coverage, all_of(feature_name)) %>%
    ...
    arrange(cluster_num, desc(cluster_coverage), GO)
...
create_scatter_plot <- function(namespace, show_legend = FALSE) {
  ...
  p <- ggplot(plot_data, aes(x = cluster_num, y = factor(unique_name, levels = rev(plot_data %>% arrange(cluster_num, desc(cluster_coverage)) %>% pull(unique_name) %>% unique())))) +
    geom_point(aes(size = cluster_coverage, color = if("p_fdr_bh" %in% colnames(plot_data)) p_fdr_bh else adj_p), alpha = 0.8) +
...
create_ridge_plot <- function(feature_name, namespace, y_levels = NULL, show_legend = TRUE) {
  ...
  if (!is.null(y_levels)) {
    ...
    ridge_data <- ridge_data %>%
      arrange(cluster_num, desc(cluster_coverage)) %>%
...
if ("BP" %in% unique(enriched_terms$NS)) {
  ...
  ggsave("/data/c/yangyusheng_optimized/DIT_HAP_pipeline/results/HD_DIT_HAP/20_gene_level_clustering/enrichment_analysis/reselected_bp_enrichment_analysis_ridge_plot.pdf", bp_plot, width = 16, height = 10)
} else {
...
summary_stats <- enriched_terms %>%
  ...
  summarise(
    ...
    median_coverage = median(cluster_coverage),
    .groups = "drop"
  ) %>%
