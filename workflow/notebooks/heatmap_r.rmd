...
```{r load_data}
# Load the data
data <- read_tsv("../../results/HD_DIT_HAP/20_gene_level_clustering/manual_cluster_labels_based_on_kmeans36.tsv")
...
data_sorted <- data %>%
  arrange(as.numeric(cluster), RevisedDeletion_essentiality, A, um, lam) %>%
...
essentiality_data <- as.character(data_sorted$RevisedDeletion_essentiality)
...
col_fun_fitted <- colorRamp2(
  c(-1, 0, 8), 
  c("#f7fcf0", "#41ab5d", "#238b45")
)

# Create individual color functions for each feature with high contrast
A_quantiles <- quantile(features[1,], c(0.05, 0.5, 0.95), na.rm = TRUE)
col_fun_A <- colorRamp2(
  c(-1, 0, 8), 
  c("#fff7ec", "#fd8d3c", "#d94801")
)

um_quantiles <- quantile(features[2,], c(0.05, 0.5, 0.95), na.rm = TRUE)
col_fun_um <- colorRamp2(
  c(-0.5, 0, 2), 
  c("#fcfbfd", "#9e9ac8", "#6a51a3")
)

lam_quantiles <- quantile(features[3,], c(0.05, 0.5, 0.95), na.rm = TRUE)
col_fun_lam <- colorRamp2(
  c(0, 6, 12), 
  c("#f7fbff", "#6baed6", "#2171b5")
)
...
cat("A quantiles (5%, 50%, 95%):", A_quantiles, "\n")
...
h_A <- Heatmap(
  ...
  heatmap_legend_param = list(title = "A", title_gp = gpar(fontsize = 10)),
  cluster_rows = FALSE,
...
features_combined <- h_A %v% h_um %v% h_lam
