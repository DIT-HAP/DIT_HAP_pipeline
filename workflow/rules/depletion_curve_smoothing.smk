...
rule data_integration:
    input:
        ...
        normalized=rules.concat_samples.output.normalized,
    ...
    output:
        output_GMs = project_dir/f"14_integrated_values/{project_name}.GMs.csv",
        output_annotations = project_dir/f"14_integrated_values/{project_name}.annotated_with_domains.csv"
    params:
        initial_timepoint=config["initial_time_point"]
    shell:
        """
        python workflow/scripts/depletion_curve_smoothing/data_integration.py \
            -M {input.M_value} \
            ...
            -d {input.domains} \
            -o {output.output_GMs} \
            -ao {output.output_annotations}
        """

rule domain_level_curve_fitting:
    input:
        annotations = rules.data_integration.output.output_annotations,
        GMs = rules.data_integration.output.output_GMs
    output:
        project_dir/f"15_curve_fitting/{project_name}.fit{{fitting}}.pred{{prediction}}.LOWESS{{LOWESS}}.fatol{{fatol}}.domain.csv"
    params:
        use_weighted_M = 1,
        # use_LOWESS = True
    threads:
    ...
    shell:
        ...
        python workflow/scripts/depletion_curve_smoothing/domain_level_curve_fitting.py \
            ...
            -GM {input.GMs} \
            -o {output} \
            -fatol {wildcards.fatol} \
            --use-weighted-M {params.use_weighted_M} \
            -f {wildcards.fitting} \
            -p {wildcards.prediction} \
            --use-LOWESS {wildcards.LOWESS}
        """
...
rule gene_level_curve_fitting:
    ...
    output:
        project_dir/f"15_curve_fitting/{project_name}.fit{{fitting}}.pred{{prediction}}.LOWESS{{LOWESS}}.fatol{{fatol}}.gene.csv"
    params:
        use_weighted_M = 1,
        # use_LOWESS = True
    threads:
