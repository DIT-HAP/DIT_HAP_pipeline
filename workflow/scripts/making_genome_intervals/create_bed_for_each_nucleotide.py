...
def main():
    ...
    representative_CDS_and_intron = extract_CDS_intron(gff_df, args.peptide)

    # chunk the representative_CDS_and_intron
    chunk_size = int(len(representative_CDS_and_intron)/args.cores)
    chunks = np.array_split(representative_CDS_and_intron, len(
        representative_CDS_and_intron) // chunk_size + 1)
    ...
    all_nucleotide_with_accumulation = pd.concat(
        accumulated_CDS_base, ignore_index=True)

    all_nucleotide_with_accumulation.to_csv(
...
def generate_nucleotide_df(chunk):
    ...
    for index, row in chunk.iterrows():
        ...
        sub_nucleotide_df = pd.concat(
            [sub_nucleotide_df, i_nucleotide_df], ignore_index=True)
