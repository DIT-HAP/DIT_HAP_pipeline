...
def piecewise_linear_function(x: np.ndarray, um: float, lam: float) -> np.ndarray:
    """
...
def objective_function(params: List[float], x: np.ndarray, y: np.ndarray, weight_values: np.ndarray) -> float:
    ...
    """
    um, lam = params
    y_fit = piecewise_linear_function(x, um, lam)
    residuals = y - y_fit
...
def fit_single_curve(x_values: np.ndarray, y_values: np.ndarray, 
    ...
    try:
        ...
        result = minimize(
            ...
            bounds=((-0.5, 2), (-1e-6, t_last)),
            options={'maxiter': 3000, 'disp': False},
        )

        if result.success:
            um, lam = result.x
            residuals = y_values - piecewise_linear_function(x_values, um, lam)
...
def main() -> None:
    ...
    output_file = Path("../tmp/Piecewise_Linear_Function_curve_fitting/Gene_level_LFC_fitted_fixA.tsv")
    ...
    numeric_columns = ['um', 'lam', 'R2', 'RMSE', 'normalized_RMSE']
